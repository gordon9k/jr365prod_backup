...
{	
	...
	public function confirm($user)
	{
		...
		try {
			...
			// $result = ['id' => $user->id, 'phone' => $user->telephone_no, 'activation_code' => $user->activation_code];
			$result = ['id' => $user->id, 'phone' => $user->telephone_no, 'activation_code' => $user->activation_code];

		} catch (\Exception $ex){
			Log::info('user register fail'.$ex);
			return response()->json(['error' => true, 'result'=>'Something wrong in registration user', 'status_code' => 210]);
		}
		return response()->json(['error' => false, 'confirm_code' => $result, 'status_code' => 200]);
	}
	...
	{	Log::info('user register start');
		try{
			...
			// Log::info('user confirmCode send');
		}catch(\Exception $ex){
			Log::info('user register fail'.$ex);
